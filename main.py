...
def pre_processing_line(config_dico):
    ...
    if config_dico['testReBlur'] :
        pre_process_output_name = f"{config_dico['pre_processed_output_path']}{config_dico['filename_no_ext']}{config_dico['rad']}Ring{config_dico['ring']}Bord{config_dico['border']}Sig{int(config_dico['sigma'])}CutOff{config_dico['cut_off']}_TestReBlur.png"
    else :
        pre_process_output_name = f"{config_dico['pre_processed_output_path']}{config_dico['filename_no_ext']}{config_dico['rad']}Ring{config_dico['ring']}Bord{config_dico['border']}Sig{int(config_dico['sigma'])}CutOff{config_dico['cut_off']}.png"
    ...
    config_dico['filename_yuv'] = config_dico['pre_processed_output_path'] + config_dico['filename'].split('.')[0] + ".yuv"
    ...
    vtm_output = f"{config_dico['pre_processed_output_path']}vtm_out_qp{config_dico['qp']}.yuv"
    ...
    config_dico['filename_rgb_out'] = config_dico['pre_processed_output_path'] + "qp_"+config_dico['qp'] + "_" +config_dico['filename'] #here it's for the output of ffmpeg2  
    ...
    if (config_dico['generate_rlc_cfg'] == 'true') :
        RLC_config_name = f"{config_dico['output_path']}preproc_RLC_config.cfg"
...
def rlc_line(config_dico):    #CHOISIR NOM POUR PSNR, ICI C'EST LA REF
    # ______________ RLC ______________
    if (config_dico['generate_rlc_cfg'] == 'true') :                                                                    
        ref_RLC_config_name = f"{config_dico['output_path']}ref_RLC_config.cfg"
...
def no_pre_processing_line(config_dico):
    config_dico['no_pre_processed_output_path'] = config_dico['output_path'] + "no_pre_processing_line/"
    # ______________ FFmpeg1 ______________
    config_dico['filename_yuv'] = config_dico['no_pre_processed_output_path'] + config_dico['filename'].split('.')[0] + ".yuv"
    ...
    # ______________ VTM ______________
    # WATCHOUT vtm only takes yuv420p and outputs only yuv420p10le
    vtm_output = f"{config_dico['no_pre_processed_output_path']}vtm_out_qp{config_dico['qp']}.yuv"
    vtm_command = f"{config_dico['vtm_path']}EncoderApp -i {config_dico['filename_yuv']} -q {config_dico['qp']} -wdt {config_dico['width']} -hgt {config_dico['height']} -c {config_dico['vtm_config_file']} -f {config_dico['frames_to_convert']} -fr {config_dico['framerate']} -o {vtm_output}"
    print(vtm_command)
    print("______________________________________")

    # ______________ FFmpeg2 ______________
    config_dico['filename_rgb_out'] = config_dico['no_pre_processed_output_path'] + "qp_"+config_dico['qp'] + "_" +config_dico['filename'] #here it's for the output of ffmpeg2  
    ffmpeg_command2 = f"ffmpeg -s {config_dico['width']}x{config_dico['height']} -pix_fmt yuv420p10le -i {vtm_output} -vframes {config_dico['frames_to_convert']} {config_dico['filename_rgb_out']}"
    print(ffmpeg_command2)
    print("______________________________________")
    ...
    # ______________ RLC ______________
    if (config_dico['generate_rlc_cfg'] == 'true') :
        proc_RLC_config_name = f"{config_dico['output_path']}no_preproc_RLC_config.cfg"
